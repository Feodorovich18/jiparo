...
formulario.addEventListener('submit', async function(event){
    ...
    try {
        ...
        if(respuestaServidor.exito){
            window.location.href = '/jiparo2/crud_usuario.html';
        } else {
...
const formulario_update = document.getElementById('form_update_usuario');

const btn_update_usuario = document.getElementById('btn_update_usuario');

formulario_update.addEventListener('submit', async function(event){
    event.preventDefault();

    //Indicador de carga
    btn_update_usuario.disabled = true;
    btn_update_usuario.textContent = 'Verificando...';
    btn_update_usuario.classList.add('opacity-50');

    const nombre = document.getElementById('update_nombre_usuario').value;
    const apellido_paterno = document.getElementById('update_apellido_paterno_usuario').value;
    const apellido_materno = document.getElementById('update_apellido_materno_usuario').value;
    const email = document.getElementById('update_email_usuario').value;

    const estatus = document.getElementById('update_estatus_usuario');


    const rol = document.getElementById('update_rol_usuario');
    const rolValue = rol.value;


    const datosUpdateUsuario = {
        nombre: nombre,
        apellido_paterno: apellido_paterno,
        apellido_materno: apellido_materno,
        email: email,
    ...
    console.log('Enviando al servidor:' + datosUpdateUsuario);

    try {
        // FETCH
        const response = await fetch('/jiparo2/api/usuarioServlet?action=update_usuario', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json', // Enviamos JSON
            },
            body: JSON.stringify(datosUpdateUsuario)  // Convertir objeto JS a texto JSON
        });
...
/*******************************************
 * ACTUALIZAR PROGRAMAS USUARIO
 ******************************************/

const formulario_update_programa = document.getElementById('form_update_usuario_programa');

const btn_update_usuario_programa = document.getElementById('btn_update_usuario_programa');
...
formulario_update.addEventListener('submit', async function(event){
    ...
    console.log('Enviando al servidor:' + datosProgramaUsuario);

    try {
        // FETCH
        const response = await fetch('/jiparo2/api/usuarioServlet?action=update_programa_usuario', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json', // Enviamos JSON
            },
...
btn_mostrar_usuarios.addEventListener('click', () => {
    ...
    fetch("/jiparo2/api/usuarioServlet?action=lista_usuarios", {
        method: 'GET',
        ...
        .then((res) => {
           ...
           const filas_usuarios = res.map(usuario => `
    ...
    `).join("");  // une todas las filas en un solo string
            ...
            document.getElementById("insert_usuario").innerHTML = filas_usuarios;
        });
});